...
let sesionData = []
//Чистка копий токенов на сервере каждые 24 часа
nodeCron.schedule('* * */23 * *', () => {
    console.log('Чистка сессионных данных каждые 24 часа ')
    sesionData = []
})
//Генерация токена
...
//Добавление токена в массив сессий или проверка его на валидность(CHECK/ADD-method)
...
//Генерация директории-/секция/подсекция/продукт/картинка
function generateProductDirectory(sectionName, subsectionName, productName) {
    let directoryUrl;
    if (sectionName && !subsectionName && !productName) { directoryUrl = 'frontend/source/sections/' + sectionName }
    if (sectionName && subsectionName && !productName) { directoryUrl = 'frontend/source/sections/' + sectionName + '/' + subsectionName }
    if (sectionName && subsectionName && productName) { directoryUrl = 'frontend/source/sections/' + sectionName + '/' + subsectionName + '/' + productName }
    return directoryUrl
}
        ...
        if (req.body.formMethod == "LOGIN") {
            if (await User.findOne({ userEmail: req.body.email })) {
                ...
                if (condidate[0].userPassword == hashPass(req.body.password, config.secretKey) && condidate[0].userEmail == req.body.email) {
                    let token = generateAccessToken(condidate[0]._id, "USER")
    ...
    try {
        if (!productKey) {
            ...
            if (req.files && dataFromInsert) {
                //--------------------Создание секций и подсекций в бд если они не существуют
                ...
                //------------------------------Создание продукта----------------------------
                if (dataFromInsert) {
                    //-------------------Создание папки товара и добавление фотографии---------------
                    ...
                    product.Photo = productURL + '/' + req.files.productImg.name
                    console.log(product)
                    await product.save()
                }
                ...
                console.log('Фотка или данные не получены')
                res.redirect('/insert.html')
            }
        }
    }
...
//Запросы на получение данных(Товары главной страницы/Каталог/"Корзина")
                    ...
                    switch (req.body.data.name) {
                        case "profileData":
                            if (await User.find({ userEmail: values.email })) {
                                ...
                                // FIO: data.personalData, Email: data.userEmail, Telephone: data.telephoneNumber
                                await User.updateOne(
                                    { _id: decoded.id },
